...
void USB::removeDevice(int port)
{
   for (int i=USB_NUMDEVICES;i>0;--i)
   {
      if (deviceTable[i].active && deviceTable[i].port == port)
      {
          deviceTable[i].active = false;
          deviceTable[i].address = 0;
          return;
      }    
   }
}
void USB::newDevice(int port)
{
    int i;
    int rcode;
...
int USB::read(usb_device * device, usb_endpoint * endpoint, uint16_t length, uint8_t * data, unsigned int nakLimit)
{
	...
	max3421e_write(MAX_REG_PERADDR, device->address);
        ...
        max3421e_write(rMODE,mode|bmHUBPRE);
...
int USB::write(usb_device * device, usb_endpoint * endpoint, uint16_t length, uint8_t * data)
{
	...
	uint32_t timeout = millis() + 3;//USB_XFER_TIMEOUT;  //reduce this in order to make more devices work on write.  some were timing out.
...
int USB::getHubDescriptor(usb_device *device, usb_hubDescriptor *descriptor)
{
	return(USB::controlRequest(device, bmREQ_GET_HUB_DESCR, USB_REQUEST_GET_DESCRIPTOR, 0x00,  0x29, 0x0000, 8, (uint8_t *)descriptor));
}
/**
...
int USB::clearPortFeature(usb_device *device,uint8_t fid, uint8_t port, uint8_t sel)
{
    return ( USB::controlRequest(device, bmREQ_CLEAR_PORT_FEATURE, USB_REQUEST_CLEAR_FEATURE, fid, 0, (((0x0000 | sel) << 8) | port), 0, 0));
}
int USB::setPortFeature(usb_device *device,uint8_t fid, uint8_t port, uint8_t sel) 
{
    return ( USB::controlRequest(device, bmREQ_SET_PORT_FEATURE, USB_REQUEST_SET_FEATURE, fid, 0, (((0x0000 | sel) << 8) | port), 0, 0));
}
int USB::getPortStatus(usb_device *device,uint8_t port, uint16_t nbytes, uint8_t* dataptr) 
{
    return ( USB::controlRequest(device, bmREQ_GET_PORT_STATUS, USB_REQUEST_GET_STATUS, 0, 0, port, nbytes, dataptr));
}
void USB::resetHubPort(usb_device *device,uint8_t port)
{
    usb_hubEvent evt;    
    clearPortFeature(device,HUB_FEATURE_C_PORT_ENABLE, port, 0);
    clearPortFeature(device,HUB_FEATURE_C_PORT_CONNECTION, port, 0);
    setPortFeature(device,HUB_FEATURE_PORT_RESET,port,0);
    for (int i=0;i<3;i++)
    {
       getPortStatus(device,port,4,evt.evtBuff);
       if (evt.bmEvent == bmHUB_PORT_EVENT_RESET_COMPLETE || evt.bmEvent == bmHUB_PORT_EVENT_LS_RESET_COMPLETE)
           break;
       delay(50);
    }
    clearPortFeature(device,HUB_FEATURE_C_PORT_RESET, port, 0);
    clearPortFeature(device,HUB_FEATURE_C_PORT_CONNECTION, port, 0);
    delay(20);
}
