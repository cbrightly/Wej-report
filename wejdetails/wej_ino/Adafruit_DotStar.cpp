...
Adafruit_DotStar::Adafruit_DotStar(uint16_t n, uint8_t o) :
 taskPos(0),numLEDs(n), dataPin(USE_HW_SPI), brightness(0), pixels(NULL),
...
Adafruit_DotStar::Adafruit_DotStar(uint16_t n, uint8_t data, uint8_t clock,
  uint8_t o) :
 taskPos(0),dataPin(data), clockPin(clock), brightness(0), pixels(NULL),
 rOffset(o & 3), gOffset((o >> 2) & 3), bOffset((o >> 4) & 3)
{
  updateLength(n);
}
...
void Adafruit_DotStar::showTask(byte brightness)
{
    if (taskPos<4)
    {
        taskPixels = pixels;
    ...
    else if (taskPos<((numLEDs+1)*4))
    {
        if (taskPos%4==0)
            sw_spi_out(448+brightness);
        else
        {
            sw_spi_out(*taskPixels++);
            
        }
    }
    else if (taskPos<((numLEDs+2)*4))
    {
        sw_spi_out(448+brightness);
    }
    taskPos++;
    if (taskPos>=((numLEDs+2)*4))
    {
        taskPos = 0;
    }
}
